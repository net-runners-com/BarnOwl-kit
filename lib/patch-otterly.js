...
// ── server/models.js patch: refresh the Claude catalog + advertise Codex ─────
// Upstream otterly advertises claude-opus-4 / claude-sonnet-4 / claude-3-5-haiku.
// Sonnet 4 was retired 2026-06-15 and 3.5 Haiku 2026-02-19, so the claude CLI
// rejects them ("you may not have access to it") — clients that populate their
// model picker from /v1/models get dead ids. Replace them with ids the current
// CLI actually accepts, then append the Codex family.
const CODEX_MODELS_MARKER = 'id: "claude-opus-5"';
...
const CODEX_MODELS_REPLACEMENT = [
  '{ id: "sonnet", label: "Claude Sonnet (default)", contextWindow: 200000 },',
  '    { id: "opus", label: "Claude Opus", contextWindow: 200000 },',
  '    { id: "haiku", label: "Claude Haiku (fastest)", contextWindow: 200000 },',
  '    { id: "fable", label: "Claude Fable 5", contextWindow: 200000 },',
...
// server/models.js: the catalog default is also a retired id — findModel() falls
// back to it for any unlisted model, so it has to be a live one too.
...
function ensurePatched() {
  ...
  // server/models.js: live Claude ids + codex models on /v1/models + /api/tags.
  ...
  const results = [
    ...
    codexImportResult, codexEngineResult, codexModelsResult, defaultModelResult,
