 ...
 * A dead login (401 / "sign in again" on stderr) → authError("codex") → HTTP 401,
 * and lib/auth-state.cjs hides the Codex models until a request succeeds again.
 *
...
/**
 * Effort levels each model accepts, lowest → highest. Verified 2026-09-18
 * against codex-cli 0.155.0 on a ChatGPT account (unsupported values are a 400):
 *   gpt-5.6-terra  none … max, plus "ultra" (max + automatic task delegation)
 *   gpt-5.6-luna   none … max
 *   gpt-5.5        none … xhigh
 * "minimal" is rejected by all three. Pre-5.5 models (API-key accounts only;
 * the ChatGPT backend no longer serves them) keep minimal … high.
 */
const EFFORT_ORDER = ["none", "minimal", "low", "medium", "high", "xhigh", "max", "ultra"];
const LEVELS_5_5 = ["none", "low", "medium", "high", "xhigh"];
const LEVELS_5_6 = [...LEVELS_5_5, "max"];
const CODEX_EFFORTS = {
  "gpt-5.6-terra": [...LEVELS_5_6, "ultra"],
  "gpt-5.6-luna": LEVELS_5_6,
  "gpt-5.5": LEVELS_5_5,
};
const CODEX_DEFAULT_MODEL = "gpt-5.6-terra"; // what `codex exec` picks with no -m

function codexEffortLevels(model) {
  const m = String(model || "").toLowerCase();
  const id = !m || m === "codex" || m === "codex-default" || m === "default" ? CODEX_DEFAULT_MODEL : m;
  if (CODEX_EFFORTS[id]) return CODEX_EFFORTS[id];
  if (/^gpt-5\.([6-9]|\d{2,})/.test(id)) return LEVELS_5_6;
  if (/^gpt-5\.5/.test(id)) return LEVELS_5_5;
  return ["minimal", "low", "medium", "high"];
}

/**
 * Clamp an effort level to what the target model accepts: none/minimal are
 * swapped for whichever of the two the model knows, anything above the model's
 * ceiling becomes its highest level, and unknown values fall back to medium.
 */
function mapCodexEffort(e, model) {
  const levels = codexEffortLevels(model);
  let x = String(e || "").toLowerCase();
  if (x === "minimal" && !levels.includes(x)) x = "none";
  if (x === "none" && !levels.includes(x)) x = "minimal";
  if (levels.includes(x)) return x;
  const rank = EFFORT_ORDER.indexOf(x);
  const top = levels[levels.length - 1];
  if (rank > EFFORT_ORDER.indexOf(top)) return top;
  return "medium";
}
...
class CodexEngine {
  ...
  async run(prompt, options) {
    ...
    if (effort) {
      args.push("-c", `model_reasoning_effort="${mapCodexEffort(effort, opts.model)}"`);
    }
    // GPT-5 verbosity (response length steering) — a real backend knob, unlike
    // temperature/top_p which the ChatGPT-Codex backend rejects outright.
    const verbosity = String(opts.verbosity || "").toLowerCase();
