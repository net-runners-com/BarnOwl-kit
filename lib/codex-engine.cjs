...
/**
 * Clamp an effort level to what the target model's backend accepts.
 * gpt-5.6+ (and the codex default, currently gpt-5.6-sol) dropped "minimal"
 * and accept none|low|medium|high|xhigh — sending "minimal" there is a 400.
 * Older gpt-5.x accept minimal|low|medium|high.
 */
function mapCodexEffort(e, model) {
  const x = String(e || "").toLowerCase();
  const m = String(model || "").toLowerCase();
  const modern = !m || m === "codex" || m === "default" || /^gpt-5\.([6-9]|\d{2,})/.test(m);
  if (modern) {
    if (x === "minimal") return "none";
    if (x === "max") return "xhigh";
    if (["none", "low", "medium", "high", "xhigh"].includes(x)) return x;
    return "medium";
  }
  if (x === "none") return "minimal";
