 ...
 *   model    (optional)  codex | gpt-5.6-terra | ... (":<effort>" suffix stripped)
 ...
 * the file saved under <state dir>/images/ (default ~/.barnowl/images/) for direct access.
 */
...
function runGenerateScript({ prompt, model, size, quality, image, signal }) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(process.env.BARNOWL_PYTHON || "python3", args, {
      env: childEnv(),
