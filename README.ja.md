...
- セッションのターンは**ウォームプロセスプール**が処理します: セッションの
  初回ターンで live な `claude` プロセスを起動（文脈ごと resume）し、以降の
  ターンはそれを再利用 — プロセス起動コストが消えます（実測: ウォームターン
  約1.8〜2.5秒 vs コールド約4〜8秒）。アイドルセッションは
  `BARNOWL_SESSION_IDLE` 秒（デフォルト600）で回収され、live プロセスは最大
  `BARNOWL_MAX_SESSIONS` 個（デフォルト8、LRU破棄）。回収後の次ターンは自動で
